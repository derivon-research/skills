...
var workspaceRoot = path.resolve(args.shift() ?? ".");
...
var maxMillis = takeValue("--max-millis") ?? "10000";
var maxReferences = takeValue("--max-references") ?? "500";
var allowApproximate = takeFlag("--allow-approximate");
...
var outputRoot = path.resolve(outputArg);
...
var manifest = JSON.parse(await readFile(path.join(workspaceRoot, ".derivon", "workspace.json"), "utf8"));
...
var solved = spawnSync("derivon", routeArgs, {
...
var objectByPublication = new Map([...objectById.values()].map((object) => [normalizeWorkspacePath(`${object.data.document}/index.html`), object]));
var sequence = [];
...
var linksById = /* @__PURE__ */ new Map();
var queue = sequence.map((entry) => objectById.get(entry.id));
var discovered = new Set(routeIds);
for (let cursor = 0; cursor < queue.length; cursor += 1) {
  ...
  let html;
  try {
    html = await readFile(sourceIndex, "utf8");
  } catch {
    fail(`Missing document for ${object.kind} ${object.id}: ${path.relative(workspaceRoot, sourceIndex)}`);
  }
  ...
  for (const link of links) {
    const direct = referrers.get(link.target.id) ?? /* @__PURE__ */ new Set();
...
async function prepareStage(output, overwrite) {
  try {
    ...
    let marker;
    try {
      marker = JSON.parse(await readFile(path.join(output, ".derivon-textbook.json"), "utf8"));
    } catch {
...
function knownObjectLinks(html, sourceObject, targets2) {
  ...
  visitHtml(document, (node) => {
    ...
    const target = resolveObjectHref(`${sourceObject.data.document}/index.html`, attribute.value, targets2);
    if (!target) return;
...
function resolveObjectHref(sourceDocument, href, targets2) {
  ...
  return targets2.get(resolved) ?? null;
}
...
function textbookIndex(manifestValue, routeValue, chapters, references2) {
  ...
  const referenceItems = references2.map((reference) => `<li><span>R</span><a href="${escapeHtml(reference.href)}">${escapeHtml(reference.title)}</a><small>${escapeHtml(reference.kind)}</small></li>`).join("\n");
