...
if (takeFlag('--help') || takeFlag('-h')) {
  ...
  node export-route-textbook.mjs <workspace> --output <directory>
    ...
    [--max-nodes <n>] [--max-millis <n>] [--allow-approximate]
    [--force] [--serve] [--port <n>]`);
  process.exit(0);
}
...
for (const entry of sequence) {
  const object = entry.kind === 'concept' ? pointById.get(entry.id) : edgeById.get(entry.id);
  ...
  try { await stat(sourceIndex); } catch { fail(`Missing document for ${entry.kind} ${entry.id}: ${path.relative(workspaceRoot, sourceIndex)}`); }
...
try {
  ...
  for (let index = 0; index < sequence.length; index += 1) {
    ...
    await writeFile(file, injectNavigation(html, previous, next), 'utf8');
  }
  ...
  await writeFile(path.join(stageRoot, 'index.html'), textbookIndex(manifest, route, sequence), 'utf8');
...
function textbookIndex(manifestValue, routeValue, chapters) {
...
function mime(file) {
  return ({ '.html': 'text/html; charset=utf-8', '.css': 'text/css; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.json': 'application/json; charset=utf-8', '.svg': 'image/svg+xml', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp', '.woff2': 'font/woff2' })[path.extname(file).toLowerCase()] ?? 'application/octet-stream';
}
